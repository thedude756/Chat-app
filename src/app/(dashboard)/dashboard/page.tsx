...
const page = async ({}) => {
    ...
    const friendsWithLastMessage = await Promise.all(
        friends.map( async (friend) => {
            const [lastMessageRaw] = await fetchRedis('zrange', `chat:${chatHrefContructor(session.user.id, friend.id)}:messages`, -1, -1) as string[]
            ...
            {friendsWithLastMessage.length === 0 ? (
                ...
                friendsWithLastMessage.map((friend) => (
                    <div key={friend.id} className='relative bg-zinc-50 border-zinc-200 p-3 rounded-md'>
                        ...
                        <Link href={`/dashboard/chat/${chatHrefContructor(
                            session.user.id,
                            friend.id
                        )}`}
